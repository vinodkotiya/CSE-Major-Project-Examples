...
If BadGuys(x).Activated = 0 Then GoTo 10
    Set BadGuys(x).PicT = Form2.PicC
    Set BadGuys(x).mask = Form2.PicCM
    BadGuys(x).bulletlxpos = 14
    BadGuys(x).bulletlypos = 58
    BadGuys(x).bulletrxpos = 64
    BadGuys(x).bulletrypos = 58
    BadGuys(x).xsize = 76
    BadGuys(x).ysize = 78
    If x > 10 Then
    ...
    If BadGuys(x).Activated = 1 And BadGuys(x).Exploding = 0 Then
       ...
       If x > 10 And numsec < 260 Then BadGuys(x).y = Form1.PicMain.ScaleTop - 150
       If BadGuys(x).y > (Form1.PicMain.ScaleTop + 30) And flgm = 0 Then
          flgm = 1
       ...
       End If
       If flgm = 0 Or flgm = 2 Then BadGuys(x).y = BadGuys(x).y + BadGuys(x).Velocity
    ...
    If x > 10 And BadGuys(x).Damage > 20 Then BadGuys(x).Exploding = 1
