...
If BadGuys(x).Activated = 0 Then GoTo 10
    Set BadGuys(x).PicT = Form2.PicH
    Set BadGuys(x).mask = Form2.PicHm
    BadGuys(x).bulletlxpos = 26
    BadGuys(x).bulletlypos = 36
    ...
    If BadGuys(x).Activated = 1 And BadGuys(x).Exploding = 0 Then
       ...
       If x > 11 And numsec < 240 Then BadGuys(x).y = Form1.PicMain.ScaleTop - 150
       If x > 13 And numsec < 280 Then BadGuys(x).y = Form1.PicMain.ScaleTop - 150
