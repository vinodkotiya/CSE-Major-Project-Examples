...
For x = 0 To CurLevel.NumOfBadGuys
   ...
   Set BadGuys(x).PicT = Form2.PicE
   Set BadGuys(x).mask = Form2.PicEm
   BadGuys(x).bulletlxpos = 36
   BadGuys(x).bulletlypos = 30
   BadGuys(x).bulletrxpos = 36
   BadGuys(x).bulletrypos = 30
   BadGuys(x).bulletcxpos = 36
   BadGuys(x).bulletcypos = 30
   BadGuys(x).xsize = 76
   BadGuys(x).ysize = 74
   If BadGuys(x).Activated = 1 And BadGuys(x).Exploding = 0 Then
'  Note the index
      If BadGuys(0).y > (Form1.PicMain.ScaleTop + 20) And _
         BadGuys(1).y > (Form1.PicMain.ScaleTop + 20) And flgm = 0 Then
         flgm = 1
      ...
      If flgm = 1 Then
         ...
         Else
            BadGuys(x).x = BadGuys(x).x + BadGuys(x).Velocity
         End If
         If numsec > 300 Then BadGuys(x).y = BadGuys(x).y + BadGuys(x).Velocity + 1
      End If
...
'Firing bullets
If BadGuys(x).y > Form1.PicMain.ScaleTop Then
For y = 0 To 4
...
If BadGuys(x).Bulletl(y).Activated = 0 Then
   ...
   BadGuys(x).Bulletl(y).x = BadGuys(x).x + BadGuys(x).bulletlxpos + Int(Rnd * (y * 7))
   BadGuys(x).Bulletl(y).y = BadGuys(x).y + BadGuys(x).bulletlypos + Int(Rnd * (y * 7))
   curX = BadGuys(x).Bulletl(y).x
   curY = BadGuys(x).Bulletl(y).y
   AngleRadians
   BadGuys(x).Bulletla(y) = Angle
End If
If BadGuys(x).Bulletr(y).Activated = 0 Then
   BadGuys(x).Bulletr(y).Activated = 1
   BadGuys(x).Bulletr(y).x = BadGuys(x).x + BadGuys(x).bulletrxpos + Int(Rnd * (y * 7))
   BadGuys(x).Bulletr(y).y = BadGuys(x).y + BadGuys(x).bulletrypos + Int(Rnd * (y * 7))
   curX = BadGuys(x).Bulletr(y).x
   curY = BadGuys(x).Bulletr(y).y
   AngleRadians
   BadGuys(x).Bulletra(y) = Angle
End If
If BadGuys(x).Bulletc(y).Activated = 0 Then
   BadGuys(x).Bulletc(y).Activated = 1
   BadGuys(x).Bulletc(y).x = BadGuys(x).x + BadGuys(x).bulletcxpos + Int(Rnd * (y * 7))
   BadGuys(x).Bulletc(y).y = BadGuys(x).y + BadGuys(x).bulletcypos + Int(Rnd * (y * 7))
...
For x = 0 To CurLevel.NumOfBadGuys
  For y = 0 To 4
   If BadGuys(x).Bulletl(y).Activated = 1 Then
      ...
      BitBlt Form1.PicScreenBuffer.hdc, BadGuys(x).Bulletl(y).x, BadGuys(x).Bulletl(y).y, 8, 8, Form2.PicA4Bullet.hdc, 0, 0, vbSrcAnd
      If Abs((BadGuys(x).Bulletl(y).x - 32) - ShipX) < 25 And Abs((BadGuys(x).Bulletl(y).y - 32) - ShipY) < 25 Then
   ...
   If BadGuys(x).Bulletr(y).Activated = 1 Then
      BadGuys(x).Bulletr(y).x = BadGuys(x).Bulletr(y).x + Cos(5 * Radians)
      ...
      BitBlt Form1.PicScreenBuffer.hdc, BadGuys(x).Bulletr(y).x, BadGuys(x).Bulletr(y).y, 8, 8, Form2.PicA4Bullet.hdc, 0, 0, vbSrcAnd
      If Abs((BadGuys(x).Bulletr(y).x - 32) - ShipX) < 25 And Abs((BadGuys(x).Bulletr(y).y - 32) - ShipY) < 25 Then
