...
If BadGuys(x).Activated = 0 Then GoTo 10
    Set BadGuys(x).PicT = Form2.PicG
    Set BadGuys(x).mask = Form2.PicGm
    BadGuys(x).bulletcxpos = 36
    BadGuys(x).bulletcypos = 28
    BadGuys(x).xsize = 71
    BadGuys(x).ysize = 50
    ...
    If BadGuys(x).Activated = 1 And BadGuys(x).Exploding = 0 Then
       BadGuys(x).y = BadGuys(x).y + BadGuys(x).Velocity + 3
       BadGuys(x).x = BadGuys(x).oldX + Sin(BadGuys(x).y / 25) * 5
...
BitBlt Form1.PicScreenBuffer.hdc, BadGuys(x).Bulletc(y).x, BadGuys(x).Bulletc(y).y, 8, 8, Form2.PicA2Bullet.hdc, 0, 0, vbSrcAnd
If BadGuys(x).Bulletc(y).x + Form2.PicA2Bullet.ScaleWidth > ShipX And BadGuys(x).Bulletc(y).x < ShipX + Form2.Picture1.ScaleWidth And _
